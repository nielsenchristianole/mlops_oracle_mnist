...
def get_schedular(optimizer, lr_half_period, lr_mult_period, lr_min, lr_warmup_max, lr_warmup_period):
    lr_cosine = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer=optimizer, T_0=int(lr_half_period), T_mult=int(lr_mult_period), eta_min=float(lr_min)
    )
